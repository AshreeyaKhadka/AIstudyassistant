...
const UploadedMaterials = ({ materials }) => {
  const getFileIcon = (type) => {
    switch(type) {
      ...
      case 'slide': return <FileImage className="text-amber-500" size={20} />;
  ...
  const getFileBadge = (type) => {
    switch(type) {
      ...
      case 'doc': return 'bg-blue-50 text-blue-600 border-blue-100/30';
      case 'slide': return 'bg-amber-50 text-amber-600 border-amber-100/30';
              ...
              <tr key={file.id} className="hover:bg-slate-50/20 transition-all group">
                <td className="px-6 py-4.5">
                  <div className="flex items-center gap-3.5">
                    <div className={`p-2.5 rounded-xl border flex items-center justify-center ${getFileBadge(file.type)} group-hover:scale-105 transition-transform duration-300`}>
                      {getFileIcon(file.type)}
                    </div>
                    <div className="min-w-0">
                      <p className="font-semibold text-slate-700 truncate max-w-[150px] sm:max-w-[200px] group-hover:text-slate-800 transition-colors">{file.filename}</p>
                ...
                <td className="px-6 py-4.5 hidden md:table-cell text-slate-400 font-bold text-xs">{file.size}</td>
                <td className="px-6 py-4.5 text-right">
                  <div className="flex justify-end gap-2 relative z-10">
                    <button className="p-2 text-slate-400 hover:text-blue-600 bg-slate-50 hover:bg-blue-50 rounded-xl border border-transparent hover:border-blue-100/30 transition-all shadow-sm" title="Preview">
                      <Eye size={14} />
                    </button>
                    <button className="p-2 text-slate-400 hover:text-blue-600 bg-slate-50 hover:bg-blue-50 rounded-xl border border-transparent hover:border-blue-100/30 transition-all shadow-sm" title="Download">
                      <Download size={14} />
                    </button>
                  </div>
                </td>
